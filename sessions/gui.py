...
class SessionCanvas(tk.Canvas):
    ...
    def init_static(self, command=None):

        self.create_rectangle(0.0, 0.0, 425.0, 832.0, fill="#FFFFFF", outline="")
        self.x_icon = ImageTk.PhotoImage(Image.open(f"{os.path.dirname(__file__)}/assets/x_icon2.png").resize((1,1)))
        ...
        for i, session_name in enumerate(self.session_list):
            ...
            self.create_window(30, 120 + i * 30, anchor="nw", window=button_2, width=300.0, height=25.0)
