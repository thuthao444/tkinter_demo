...
class ToggleButton(tk.Canvas):
    ...
    def _resize(self, event):
        ...
        # Đẩy nút lên 1 pixel (giảm toạ độ y xuống 1)
        btn_y = padding - 1
        btn_x = width - padding - arc_diameter if self.state else padding
        self.coords(self.btn, btn_x, btn_y, btn_x + arc_diameter, btn_y + arc_diameter)
    ...
    def _animate_knob(self):
        ...
        arc_diameter = height - 2 * padding
        target_x = width - padding - arc_diameter-2 if self.state else padding-2
        
        # Giữ y cố định, đẩy lên 1 pixel
        btn_y = padding - 1
        ...
        if abs(current_x - target_x) < 2:
            self.moveto(self.btn, target_x, btn_y)
            return

        step = 2.5 if target_x > current_x else -2.5
        new_x = current_x + step
        self.moveto(self.btn, new_x, btn_y)
